...
const (
	...
	SharedLogOpType_READ_PREV_AUX uint16 = 0x21
	SharedLogOpType_READ_LOCALID  uint16 = 0x22
)
...
func NewSharedLogReadMessage(currentCallId uint64, myClientId uint16, tag uint64, seqNum uint64, direction int,
	block bool, promiseAux bool, clientData uint64) []byte {
	...
	if direction > 0 {
		...
		if promiseAux {
			binary.LittleEndian.PutUint16(buffer[32:34], SharedLogOpType_READ_PREV_AUX)
		} else {
