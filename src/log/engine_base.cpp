...
EngineBase::EngineBase(engine::Engine* engine)
    : node_id_(engine->node_id_),
      engine_(engine),
      next_local_op_id_(0),
      syncto_counter_(stat::Counter::StandardReportCallback("log_syncto")),
      read_counter_(stat::Counter::StandardReportCallback("log_read")),
      syncto_delay_stat_(stat::StatisticsCollector<int32_t>::StandardReportCallback(
          "log_syncto_delay")),
...
void EngineBase::RecordOpDelay(protocol::SharedLogOpType op_type, int32_t delay) {
    ...
    case SharedLogOpType::ASYNC_READ_LOCALID:
        break;
    case SharedLogOpType::READ_SYNCTO:
        syncto_delay_stat_.AddSample(delay);
        break;
    case SharedLogOpType::TRIM:
        break;
    case SharedLogOpType::SET_AUXDATA:
